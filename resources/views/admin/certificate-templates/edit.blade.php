...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold text-dark mb-0"><i class="bi bi-pencil-square me-2"></i>Edit Template Sertifikat</h2>
                    ...
                    <form method="post" action="{{ route('admin.templates.update', $template) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="html_template" class="form-control form-control-lg @error('html_template') is-invalid @enderror"
                                rows="12" placeholder="Masukkan HTML template sertifikat Anda di sini..." required>{{ old('html_template') ?? $template->html_template }}</textarea>
                        ...
                        <div class="mb-4">
                            <label class="form-label fw-bold"><i class="bi bi-pen me-2"></i>Tanda Tangan Digital</label>
                            @if ($template->signature_path)
                                <div class="alert alert-info border-0 mb-3">
                                    <div class="small text-muted fw-bold mb-2">Tanda tangan saat ini:</div>
                                    <img src="{{ asset('storage/' . $template->signature_path) }}" alt="Signature" style="max-height: 80px;" class="border rounded p-2">
                                </div>
                            @endif
                            <input type="file" name="signature" class="form-control form-control-lg" accept="image/*">
                            <small class="form-text text-muted d-block mt-2"><i class="bi bi-info-circle me-1"></i>Format: PNG, JPG (Max 2MB) - Kosongkan jika tidak ingin diubah</small>
                        </div>
