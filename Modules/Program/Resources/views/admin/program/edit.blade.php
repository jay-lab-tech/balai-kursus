                ...
                <form method="POST" action="{{ route('admin.program.update', $program->id) }}">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" id="nama" name="nama" value="{{ old('nama', $program->nama) }}" required>
                        </div>

                        <div>
                            <label for="warna" class="block text-sm font-medium text-gray-700">Warna Program</label>
                            <input type="color" class="mt-1 h-12 w-full rounded-xl border border-gray-300 shadow-sm" id="warna" name="warna" value="{{ old('warna', $program->warna ?? '#eab308') }}">
                            <p class="mt-2 text-xs text-gray-500">Warna ini dipakai untuk kartu program di halaman peserta.</p>
                            <div class="mt-3 rounded-2xl p-4 text-white shadow-sm" style="background: linear-gradient(135deg, {{ old('warna', $program->warna ?? '#eab308') }}, #111827);">
